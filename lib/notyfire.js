...
var NotyfireServer = function(port, redis_url, redis_channel) {
    // Config
    this.port = port;

    // Redis
    this.redis_client = null;
    this.redis_url = redis_url ? redis_url : '';
    this.redis_channel = redis_channel ? redis_channel : REDIS_PUBSUB_CHANNEL;
...
NotyfireServer.prototype.dispatch = function(channel_id, data) {
    var client_data = {};
    client_data[channel_id] = data;
    this.io.sockets.json.in(channel_id).send(client_data);
};


NotyfireServer.prototype.run = function() {
    // Start socket.io
    this.io = socket_io.listen(this.port);
